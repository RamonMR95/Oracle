...
CREATE TABLE PERIODISTA_PLANTILLA(
	...
	CONSTRAINT p_plantilla_fk_p_plantilla
	  ...
	  ON DELETE SET NULL,-- ON UPDATE NOACTION
...
CREATE TABLE NUMERO_REVISTA(
	...
	fecha DATE NOT NULL,
	publicacion VARCHAR2(30) NOT NULL,
	revista CHAR(9) NOT NULL,
...
CREATE TABLE SECCION_REVISTA(
	...
	CONSTRAINT s_revista_pk PRIMARY KEY (codigo, num, revista),
	CONSTRAINT s_revista_fk_n_revista
		FOREIGN KEY ( num,revista)
		REFERENCES NUMERO_REVISTA(numero, revista)
		ON DELETE CASCADE --ON UPDATE CASCADE

);	
/* Atributo calculado secciones */ 
UPDATE NUMERO_REVISTA
SET secciones = ( SELECT COUNT (secciones)
				FROM SECCION_REVISTA
				WHERE identificador = num);
...
CREATE TABLE ARTICULO(
	...
	CONSTRAINT articulo_fk_s_revista
		FOREIGN KEY(seccion, num, revista)
		REFERENCES SECCION_REVISTA(codigo, num, revista)
